...
var addFolderArgS = addFolderArg{}
...
Create a new Folder, with an optional name and parent folder ID
	`,
	Aliases: []string{"createFolder", "folderAdd"},
	Example: `
...
rspace eln addFolder --name MyFolder --folder FL1234
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		doAddFolder(ctx, addFolderArgS, post)
	},
}

func doAddFolder(ctx *Context, args addFolderArg, post rspace.FolderPost) {
	if len(args.Name) > 0 {
		post.Name = args.Name
	}
...
func folderToTable(ctx *Context, folder *rspace.Folder) {
	headers := []columnDef{columnDef{"Id", 8}, columnDef{"GlobalId", 10}, columnDef{"Name", 25}, columnDef{"ParentFolderId", 15}, columnDef{"zd", 24}}
...
func init() {
	elnCmd.AddCommand(addFolderCmd)
	addFolderCmd.Flags().StringVarP(&addFolderArgS.Name, "name", "n", "", "A name for the folder")
	addFolderCmd.Flags().StringVarP(&addFolderArgS.ParentFolder, "folder", "p", "", "An id for the folder that will contain the new folder")
}
